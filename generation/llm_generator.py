...
class LLMGenerator:
    ...
    def _normalize_contexts(cls, contexts: list[dict]) -> list[dict]:
        ...
        for idx, ctx in enumerate(contexts or [], start=1):
            ...
            normalized.append({
                "title": clean_display_text(ctx.get("title") or f"Quelle {idx}"),
                "link": link,
                "summary": summary,
    ...
    def _confirmed_point(ctx: dict) -> str:
        ...
        if "model release notes" in haystack and "gpt-5.5 instant" in haystack:
            return f"- {title}: OpenAI dokumentiert ein GPT-5.5-Instant-Update für bessere Antwortqualität, Lesbarkeit und Alltagshilfe."
        if "gpt-5.5 in chatgpt" in haystack or "gpt-5.5 instant in chatgpt" in haystack:
            return f"- {title}: OpenAI beschreibt GPT-5.5 Instant als aktuellen bestätigten ChatGPT-Stand für angemeldete Nutzer."

        words = summary.split()
        if not words:
            return f"- {title}: Quelle enthält relevante Informationen, Details siehe Quellenliste."
    ...
    def _speculative_points(cls, contexts: list[dict]) -> list[str]:
        ...
        for idx, ctx in enumerate(contexts, start=1):
            ...
            if "codex" in combined and any(term in combined for term in ("log", "backend", "rollout", "mapping", "canary")):
                add(f"{marker} Codex-/Backend-Leak: Drittquellen berichten von einem kurz sichtbaren Rollout-/Mapping-Eintrag mit `gpt-5.6`, der später verschwand; das ist kein offizieller OpenAI-Nachweis.")
            ...
            if "polymarket" in combined or "prediction market" in combined or "odds" in combined:
                if re.search(r"80\s*[-\u2013]\s*89\s*%", combined):
                    add(f"{marker} Prediction-Market: Polymarket-Odds werden mit 80-89% für eine öffentliche Veröffentlichung bis 30. Juni 2026 genannt; das ist eine Markt-Wette, keine Roadmap.")
                elif "june 30" in combined:
                    add(f"{marker} Prediction-Market: Drittquellen nennen hohe Polymarket-Odds für einen Release bis 30. Juni 2026; das bleibt unbestätigt.")
                elif "july 31" in combined:
                    add(f"{marker} Prediction-Market: Einzelne Quellen nennen auch Markt-Wetten für spätestens 31. Juli 2026; das bleibt unbestätigt.")
                else:
                    add(f"{marker} Prediction-Market: Es gibt Markt-/Odds-Spekulationen zu GPT-5.6, aber keine offizielle Terminbestätigung.")
            if "mid-to-late june" in combined:
                add(f"{marker} Release-Gerücht: Eine Quelle nennt Mitte bis Ende Juni 2026 als realistisches Fenster; das ist Spekulation.")
            if "within weeks" in combined:
                add(f"{marker} Release-Gerücht: Eine Quelle formuliert, GPT-5.6 könnte innerhalb weniger Wochen erscheinen; das ist nicht bestätigt.")
            if any(term in combined for term in ("1.5m", "1.5 million", "1,5", "context window", "token context")):
                add(f"{marker} Feature-Gerücht: Ein größeres Kontextfenster bzw. 1.5M-Token-Kontext wird erwähnt, aber nicht offiziell belegt.")
            if any(term in combined for term in ("codename", "pricing", "benchmark")):
                add(f"{marker} Nicht belegte Details: Codenames, Preise und Benchmark-Werte werden ausdrücklich als nicht dokumentiert bzw. spekulativ behandelt.")
            if "advanced reasoning" in combined or "multi-step reasoning" in combined:
                add(f"{marker} Feature-Gerücht: Besseres Reasoning bzw. mehrstufiges Schlussfolgern wird behauptet, bleibt aber unbestätigt.")
            if "agentic workflow" in combined or "agentic workflows" in combined:
                add(f"{marker} Feature-Gerücht: Agentic Workflows werden als erwartete Verbesserung genannt, aber nicht offiziell bestätigt.")
            if "token efficiency" in combined:
                add(f"{marker} Feature-Gerücht: Verbesserte Token-Effizienz wird behauptet, bleibt aber unbestätigt.")
            if "dual-version" in combined or "pro" in combined:
                add(f"{marker} Feature-Gerücht: Eine Pro-/Dual-Version wird erwähnt, ist aber nicht offiziell bestätigt.")

        if points:
            return points
        return [
            f"[{idx}] {ctx.get('title', 'Quelle')}: als {credibility_label(ctx)} behandeln, nicht als bestätigte Produktinformation."
            for idx, ctx in enumerate(contexts, start=1)
    ...
    def _fallback_answer(question: str, contexts: list[dict]) -> str:
        ...
        if versioned_openai and not any("5.6" in f"{ctx['title']} {ctx['summary']}".lower() for ctx in official):
            if speculative:
                lines.append(
                    "Offiziell gibt es keine bestätigte ChatGPT-/GPT-5.6-Ankündigung. "
                    "Für deine Frage nach neuen Features und einem möglichen Release heißt das: "
                    "Man kann nur die aktuelle Quellenlage einordnen. Es gibt unbestätigte Drittquellen "
                    "zu Codex-/Backend-Spuren, Markt-Wetten und möglichen Funktionsrichtungen, aber "
                    "Release-Datum und konkrete 5.6-Features bleiben Spekulation."
                )
            else:
                lines.append(
                    "Offiziell gibt es keine bestätigte ChatGPT-/GPT-5.6-Ankündigung. "
                    "Release-Datum und konkrete 5.6-Features dürfen daher nicht als Fakt dargestellt "
                    "werden; belastbar ist nur, was die bereitgestellten Quellen ausdrücklich belegen."
                )
        else:
            lines.append(
                "Die Antwort stützt sich auf die bereitgestellten Quellen und trennt belegte Aussagen "
                "von unbestätigten Hinweisen."
            )
        ...
        if confirmed_points:
            ...
            lines.append("In den bereitgestellten Quellen gibt es keine bestätigten Informationen, die die Frage direkt beantworten.")
        ...
        if asks_features:
            ...
            if versioned_openai:
                ...
                paragraph = (
                    "Für ChatGPT/GPT-5.6 liegt keine bestätigte Feature-Liste in den offiziellen Quellen vor. "
                )
                if feature_points:
                    paragraph += (
                        "Unbestätigt werden in Drittquellen vor allem folgende Richtungen diskutiert: "
                        f"{join_points(feature_points, limit=5)} "
                    )
                if official:
                    paragraph += (
                        "Als aktueller bestätigter Stand sind nur die genannten ChatGPT-/GPT-5.5-Änderungen "
                        "belastbar; daraus darf keine 5.6-Zusage abgeleitet werden."
                    )
                lines.append(paragraph)
            elif not official:
                lines.append("Eine belastbare Feature-Liste lässt sich aus den vorhandenen Quellen nicht ableiten, weil sie nicht offiziell sind.")
            else:
                lines.append("Neue Funktionen sollten nur aus bestätigten Quellen abgeleitet werden; nicht belegte Features bleiben offen.")
        ...
        if asks_release or versioned_openai:
            ...
            if versioned_openai and not official_mentions_version:
                ...
                paragraph = "Kein offizielles Release-Datum für ChatGPT/GPT-5.6 in den bereitgestellten offiziellen Quellen. "
                if release_points:
                    paragraph += (
                        "Unbestätigt gibt es aber Terminsignale in Drittquellen: "
                        f"{join_points(release_points, limit=4)} "
                    )
                paragraph += "Diese Hinweise sind nützlich für die Gerüchte- und Marktlage, ersetzen aber keine OpenAI-Roadmap."
                lines.append(paragraph)
            else:
                lines.append("Bestätigte Termine sind nur aus den oben genannten Quellen ableitbar; nicht belegte Release-Fenster bleiben offen.")

        if speculative:
            lines.append("\n## Diskutierte, unbestätigte Hinweise")
            lines.append(
                "Die wichtigsten unbestätigten Signale sind: "
                f"{join_points(speculative_points, limit=8)} "
                "Diese Hinweise dürfen nur als Quellenlage/Spekulation erscheinen, nicht als Produktfakt."
            )

        lines.append("\n## Quellenlage")
        source_parts = []
        if official:
            source_parts.append(f"{len(official)} offizielle Quelle(n) mit höchster Priorität")
        if speculative:
            source_parts.append(f"{len(speculative)} unbestätigte Quelle(n), geeignet für Gerüchte-/Marktlage, nicht für Fakten")
        if third_party:
